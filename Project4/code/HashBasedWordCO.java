...
public class HashBasedWordCO {
	...
	public void fillCoMat(String text, String[] keyWords) {
		...
		for(String keys: coMat.keySet()) {
			for(String keys2 : coMat.keySet()) {
				for(String sentence : sentences) {
					String[] wordzz = sentence.split("\\s+");
					for(int i=0; i<wordzz.length; i++) {
						if(wordzz[i].equals(keys)) {
							for(int j=0; j<windowSize; j++) {
								if(i+j+1 < wordzz.length) {
									if(wordzz[i+j+1].equals(keys2)) {
										coMat.get(keys).increment(keys2);
									}
								}
								if(i-j-1 >= 0) {
									if(wordzz[i-j-1].equals(keys2)) {
		...
		//		for(String key : coMat.keySet()) {
		//			for(String sentence : sentences) {
		//				String[] sentWords = sentence.split("\\s+");
		//				for(String word : words.keySet()) {
		//					for(int i=0; i<sentWords.length; i++) {
		//						if(sentWords[i].equals(key)) {
		//							for(int j=i-windowSize; j<i+windowSize; j++) {
		//								if(j>0 && j<sentence.length()) {
		//									if(sentWords[j].equals(key)) {
		//										coMat.get(key).increment(word);
		//									}
		//								}
		//							}
		//						}
